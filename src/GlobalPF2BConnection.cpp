...
void GlobalPF2BConnection::init(AurynFloat tau_hom, AurynFloat eta, AurynFloat kappa, AurynFloat maxweight)
{
...
void GlobalPF2BConnection::init_shortcuts() 
{
...
void GlobalPF2BConnection::init_has_fwd_prop()
{
...
void GlobalPF2BConnection::finalize() {
	DuplexConnection::finalize();
	init_shortcuts();
}

void GlobalPF2BConnection::free()
{
}

GlobalPF2BConnection::GlobalPF2BConnection(SpikingGroup * source, NeuronGroup * destination, TransmitterType transmitter) : DuplexConnection(source, destination, transmitter)
{
	init_has_fwd_prop();
}

GlobalPF2BConnection::GlobalPF2BConnection(SpikingGroup * source, NeuronGroup * destination, 
		const char * filename, 
		AurynFloat tau_hom, 
		AurynFloat eta, 
		...
		name)
{
	init(tau_hom, eta, kappa, maxweight);
	if ( name.empty() ) 
		set_name("GlobalPF2BConnection");
	init_shortcuts();
	init_has_fwd_prop();
}

GlobalPF2BConnection::~GlobalPF2BConnection()
{
	if ( dst->get_post_size() > 0 ) 
		free();
}

inline AurynWeight GlobalPF2BConnection::dw_pre(NeuronID post)
{
	double dw = (tr_post->get(post)+xi);
	return dw;
}

inline AurynWeight GlobalPF2BConnection::dw_post(NeuronID pre)
{
...
void GlobalPF2BConnection::propagate_forward()
{
...
inline void GlobalPF2BConnection::propagate_backward()
{
...
void GlobalPF2BConnection::propagate()
{
	propagate_forward();
	propagate_backward();
}

void GlobalPF2BConnection::evolve()
{
	post_factor *= post_factor_mul;
	post_factor += dst->get_spikes()->size()/tau_post;
}

bool GlobalPF2BConnection::write_to_file(string filename)
{
...
bool GlobalPF2BConnection::load_from_file(string filename)
{
...
void GlobalPF2BConnection::log_has_fwd_prop(std::string stage)
{
	std::stringstream oss;
	if ( !stage.empty() ) {
...
void GlobalPF2BConnection::block_pre_neurons(std::string filename, int nb_max_patterns)
{
